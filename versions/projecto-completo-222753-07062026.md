...
A validação antropométrica implementada no sistema deve ser entendida como validação interna de coerência dos dados e das suas consequências paramétricas, não como validação clínica da prótese final. O objetivo é verificar se as medições recebidas podem ser normalizadas, interpretadas e transformadas em parâmetros geométricos plausíveis antes de serem usadas pela IA ou pelo modelo OpenSCAD. Esta distinção é metodologicamente essencial: a plataforma consegue assinalar medidas ausentes, intervalos anómalos e incompatibilidades entre campos, mas não consegue, por si só, provar conforto, segurança, desempenho funcional ou adequação terapêutica.

O fluxo começa pela normalização das entradas. As medições podem ser introduzidas manualmente ou importadas em formatos estruturados, sendo convertidas para milímetros e mapeadas para uma nomenclatura canónica. Termos como largura da palma, largura da mão ou largura metacarpal passam a alimentar o mesmo campo operacional; comprimentos totais dos dedos, medidas segmentares das falanges, circunferência do punho e medidas do membro residual são igualmente reorganizados numa estrutura hierárquica. Esta normalização reduz ambiguidades terminológicas e torna possível aplicar as mesmas regras a perfis provenientes de fontes diferentes.

Depois da normalização, o sistema aplica uma primeira camada de validação por intervalos e por coerência entre campos. Cada medida relevante tem limites mínimos e máximos esperados; valores fora desses limites são preservados, mas assinalados como potenciais outliers. Para além disso, existem verificações relacionais: a largura da palma não deve exceder de forma desproporcionada o comprimento da palma, a espessura não deve ser excessiva face à largura, a circunferência distal do membro residual não deve ultrapassar a proximal sem indicação de orientação invertida, e a soma das falanges deve permanecer coerente com o comprimento total do dedo quando ambos são fornecidos. Este tipo de validação não elimina a necessidade de revisão humana, mas torna explícitas incongruências que poderiam passar diretamente para a geometria.

A etapa seguinte consiste na derivação de parâmetros ausentes ou secundários. Quando só existe o comprimento total de um dedo, o sistema estima os segmentos falângicos por rácios anatómicos; quando falta uma medida estrutural da palma, usa relações proporcionais de recurso; quando existem circunferências do membro residual, calcula diâmetros internos, profundidade de encaixe e inclinação aproximada. São ainda gerados parâmetros de fabrico, como folgas, diâmetro de pivots, diâmetro de canais internos e zonas de reforço. Esta derivação torna a plataforma mais tolerante a perfis incompletos, mas também introduz incerteza. Por isso, o sistema regista quais valores foram medidos e quais foram derivados, permitindo que a IA e o técnico distingam dado primário de estimativa.

A integração com a IA ocorre sobre esta estrutura já validada. O sistema não envia à IA um conjunto indiferenciado de números; envia um contexto que inclui a fonte da medição, os campos em falta, a incerteza estimada, os outliers detetados, as preferências de tolerância e hardware e as notas sobre segmentos derivados. A IA pode, assim, produzir sugestões mais situadas: por exemplo, recomendar cautela quando várias dimensões foram inferidas, propor revisão de uma medida sinalizada como atípica, ou explicar porque determinado modelo usa uma medida apenas como contexto e não como transformação ativa. Esta arquitetura reforça a rastreabilidade porque cada sugestão pode ser lida contra os dados, os limites e as decisões de derivação que a antecedem.

Esta validação também se articula com a integração dos modelos de mão descrita no capítulo anterior. No Paraglider, por exemplo, a largura da palma é adequada para controlar a escala uniforme da palma, enquanto os comprimentos individuais dos dedos podem controlar escalas próprias das falanges e pontas digitais. Já a espessura e o comprimento da palma, embora antropometricamente relevantes, não devem deformar diretamente a palma nesse modelo específico, porque isso comprometeria furos de pinos e interfaces mecânicas. Esta decisão mostra que a validação antropométrica não consiste apenas em aceitar ou rejeitar valores: consiste em decidir que medidas são ativas, contextuais ou bloqueadas em cada família geométrica.

A principal vantagem desta abordagem é tornar o processo de personalização mais auditável. Um perfil antropométrico deixa de ser apenas uma lista de medidas e passa a ser um conjunto interpretado, com campos normalizados, valores derivados, alertas e nível de incerteza. A IA entra nesse processo como camada de leitura e apoio à decisão, não como fonte autónoma de verdade. Deste modo, a validação antropométrica assistida por IA contribui para reduzir erros de transcrição, detetar incoerências e orientar a parametrização inicial, mantendo a responsabilidade final no técnico, no designer ou no investigador que avalia a solução.
