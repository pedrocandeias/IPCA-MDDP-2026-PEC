#!/usr/bin/env python3
"""Delimita a síntese sobre abandono protésico na Secção 2.1."""
...
OLD_TEXT = (
    "Apesar da evolução tecnológica, as taxas de abandono permanecem elevadas. A "
    "literatura associa recorrentemente a rejeição protésica a problemas de conforto, "
    "peso, funcionalidade insuficiente, manutenção exigente e controlo pouco intuitivo. "
    "Esta persistência indica que a melhoria tecnológica isolada não garante adopção "
    "sustentada. Ainda assim, quando o dispositivo está adequadamente prescrito, ajustado "
    "e acompanhado, a utilização continuada de prótese tende a associar-se a maior "
    "independência funcional e a melhores indicadores de participação e de qualidade de "
    "vida do que a não utilização (Fink & Diamond, 2023; Smail et al., 2021)."
)
OLD_TEXT_DOCX = (
    "Apesar da evolução tecnológica, as taxas de abandono permanecem elevadas. A "
    "literatura associa de forma recorrente a rejeição protésica a problemas de conforto, "
    "peso, funcionalidade insuficiente, manutenção exigente e controlo pouco intuitivo. "
    "Esta persistência indica que a melhoria tecnológica isolada não garante adoção "
    "sustentada. Ainda assim, quando o dispositivo está adequadamente prescrito, ajustado "
    "e acompanhado, a utilização continuada de prótese tende a associar-se a maior "
    "independência funcional e a melhores indicadores de participação e de qualidade de "
    "vida do que a não utilização (Fink & Diamond, 2023; Smail et al., 2021)."
...
NEW_PARTS = (
    (
        "Apesar da evolução tecnológica, o abandono de próteses de membro superior "
        "continua documentado. Smail et al. (2021) agrupam as razões identificadas "
        "sobretudo em duas áreas — conforto e função — e assinalam o peso, a temperatura "
        "e a transpiração entre os problemas de conforto mais persistentes. Biddiss et "
        "al. (2007) e Fink e Diamond (2023) acrescentam que o peso, o conforto, o controlo "
        "e o ajuste do encaixe são aspectos relevantes nas prioridades dos utilizadores e "
        "no processo de escolha e acompanhamento da prótese. Em conjunto, estes resultados "
        "mostram que a melhoria tecnológica, por si só, não garante a utilização continuada "
        "e que as decisões de ",
        False,
    ),
    ("design", True),
    (" devem considerar a experiência física e funcional do utilizador.", False),
)
...
def replace_docx_paragraph(root: etree._Element) -> None:
    matches = [
        ...
        if text_of(paragraph) == OLD_TEXT_DOCX
    ]
    if len(matches) != 1:
...
def update_markdown() -> str:
    ...
    markdown = markdown.replace(
        "Versão do documento: 0.4.90", "Versão do documento: 0.4.91", 1
    )
    return markdown.replace(OLD_TEXT, NEW_MD, 1)
...
def main() -> None:
    ...
    body = text_of(root)
    if OLD_TEXT_DOCX in body or body.count(NEW_TEXT) != 1:
    ...
    print(
        "Síntese sobre abandono protésico delimitada; "
