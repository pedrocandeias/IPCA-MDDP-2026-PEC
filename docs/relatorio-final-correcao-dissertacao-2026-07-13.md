...
| --- | --- | --- | --- |
| Caso numérico completo, desde o perfil até à geometria | Capítulo 4, após a explicação do algoritmo de correspondência | Apresentar um perfil simulado, a fonte antropométrica seleccionada, os valores de entrada, as conversões, os parâmetros derivados, os limites aplicados, as variáveis OpenSCAD afectadas e as dimensões finais da malha. | Dados e cálculos guardados para um dos cenários já avaliados. |
...
| Registo completo das impressões | Capítulo 3 para o protocolo; Capítulo 8 ou apêndice para os resultados | Atribuir um código a cada peça e indicar modelo, idade simulada, impressora, programa de fatiamento, material, perfil usado, camada, paredes, enchimento, orientação, suportes, temperatura, tempo, massa, falhas e observações de montagem. Explicar que os três projectos arquivados forneceram definições comuns às restantes impressões. | Projectos 3MF, fotografias, histórico das impressoras e notas do processo. Campos sem registo devem ser assinalados como indisponíveis. |
