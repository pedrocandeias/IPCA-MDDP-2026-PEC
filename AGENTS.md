...
Before each substantive writing pass on the main manuscript, create a timestamped copy under `docs/versoes/backups/`. Use the date and time in the filename so each writing version is recoverable and ordered chronologically.
Whenever you back up the root `pedro-candeias-projeto-mestrado-mdddp-ipca-2026-revisto.docx`, also regenerate the matching root-level PDF so it always tracks the latest document. Use `tools/backup_docx.sh [label]`, which performs both steps and writes the timestamped backup under `docs/versoes/backups/`.
The PDF is also kept in sync automatically by a version-controlled git `pre-commit` hook (`tools/hooks/pre-commit`): whenever the canonical revised DOCX is part of a commit, the hook regenerates the matching PDF and adds it to the same commit. Install the hooks once per clone with `tools/install_hooks.sh` (sets `core.hooksPath` to `tools/hooks`). Do not bypass it with `git commit --no-verify` when the DOCX changed.
...
When exporting the main manuscript, create the timestamped export under `docs/versoes/exportacoes/` and create or replace the canonical root DOCX/PDF pair.
Keep an explicit version line at the top of `pedro-candeias-projeto-mestrado-mdddp-ipca-2026-revisto.md` and bump that version after every manuscript change. Reflect the same manuscript version bump in `CHANGELOG.md`.
All text written or revised in `pedro-candeias-projeto-mestrado-mdddp-ipca-2026-revisto.md` must use European Portuguese (`português de Portugal`) consistently.
...
After adding bibliography entries for sources cited in the canonical manuscript, locate the corresponding local PDF files when available and copy them into `projecto_completo_bibliografia/` so the manuscript bibliography and the supporting PDFs remain collocated for review.
Whenever citations or bibliography entries are added or changed in the canonical manuscript, update `projecto_completo_bibliografia/pdfs_em_falta.md` so it remains the authoritative list of bibliography items that still lack a local PDF in `material/` or `projecto_completo_bibliografia/`.
