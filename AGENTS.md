...
After adding bibliography entries for sources cited in the canonical manuscript, locate the corresponding local PDF files when available and copy them into `material/bibliografia/` so the manuscript bibliography and the supporting PDFs remain collocated for review.
Whenever citations or bibliography entries are added or changed in the canonical manuscript, update `material/bibliografia/pdfs_em_falta.md` so it remains the authoritative list of bibliography items that still lack a local PDF in `material/` or `material/bibliografia/`.
