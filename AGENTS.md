...
Maintain a root `CHANGELOG.md` and update it after every repository change, with a short dated note describing what changed.
...
For Elicit agent sessions, use `sources/elicit/agent-sessions/<chapter>/<subsection>/<session-slug>/` and preserve both the raw export files and extracted readable text. When available, keep generated artifacts, chat or transcript exports, and APA/citation files together in the same session folder so they can later support manuscript drafting.
