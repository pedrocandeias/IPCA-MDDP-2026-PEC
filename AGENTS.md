...
Whenever you back up the root `projecto-completo.docx`, also regenerate the root-level `projecto-completo.pdf` from the current DOCX so the PDF always tracks the latest document. Use `tools/backup_docx.sh [label]`, which performs both steps: it writes a timestamped backup to `versions/projecto-completo-docx-<timestamp>-<label>.docx` and rebuilds `projecto-completo.pdf` (via `tools/docx_to_pdf.sh`, headless LibreOffice).
The PDF is also kept in sync automatically by a version-controlled git `pre-commit` hook (`tools/hooks/pre-commit`): whenever `projecto-completo.docx` is part of a commit, the hook regenerates `projecto-completo.pdf` and adds it to the same commit. Install the hooks once per clone with `tools/install_hooks.sh` (sets `core.hooksPath` to `tools/hooks`). Do not bypass it with `git commit --no-verify` when the DOCX changed.
