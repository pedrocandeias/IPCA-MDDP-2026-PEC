...
- `figura_c1_fluxo_adaptacao_parametrica.svg`: diagrama vectorial editável de produção própria usado na secção C.2;
- `figura_c1_fluxo_adaptacao_parametrica.png`: derivado de apresentação usado na exportação DOCX/PDF, gerado a partir do SVG.
...
O directório `dicionario_parametros_v14.67.0` permanece a fonte do dicionário histórico de 42 parâmetros e do exemplo numérico Flexy Beast. O Anexo C não os substitui: documenta alterações posteriores, inclui o Cyborg Beast e explicita excepções e limitações descobertas na leitura dos modelos.
